         ...
         <main>
          ...
          <div class="row">
            ...
            <?php
				if (isset($_SESSION['username'])) {
					if($privilege == 1){
					$stmt = $connection->prepare('SELECT * FROM forms');
					
          }elseif ($privilege == 2 || $privilege == 3) {
            // Privilege level 2 or 3: Display information based on user role
            $stmt = $connection->prepare("SELECT * FROM forms WHERE Preparateur = ? OR Revision = ?");
          ...
          $stmt->execute();
			...
			?>
					 <table id="bootstrap-table" class="table table-bordered">
                      ...
                      <thead >
                        <tr>
                        <th width="100" data-sortable="true">Bureau</th>
                        <th data-sortable="true" scope="col">Nom de dossier</th>
                        <th data-sortable="true" scope="col">Associer</th>
                        <th data-sortable="true" scope="col">Personne RS</th>
                        <th data-sortable="true" scope="col">Num dossier</th>
                        <th data-sortable="true" scope="col">Projet</th>
                        <th data-sortable="true" scope="col">Fin d'année</th>
                        <th data-sortable="true" scope="col">Date de réception</th>
                        <th data-sortable="true" scope="col">Priorité</th>
                        <th data-sortable="true" scope="col">Budget d'heures</th>
                        ...
                        <th data-sortable="true" scope="col">Type de dossier</th>
                    ...
                    echo '<td>' . $Nom_doss . '</td>';
                    echo '<td>' . $Associer . '</td>';
                    echo '<td>' . $Personne_RS . '</td>';
                    echo '<td>' . $Num_dossier . '</td>';
                    echo '<td>' . $Projet . '</td>';
                    echo '<td>' . $Fin_annee . '</td>';
                    echo '<td>' . $Date_recep . '</td>';
                    // Determine the CSS class for the "Priorité" cell based on the value
                    $prioriteClass = '';
                    switch ($Priorite) {
                      case 'Haute':
                        $prioriteClass = 'badge bg-danger';
                        break;
                      case 'Faible':
                        $prioriteClass = 'badge bg-light';
                        break;
                      case 'Normal':
                        $prioriteClass = 'badge bg-secondary';
                        break;
                      default:
                        $prioriteClass = ''; // No special class for other cases
                    }
                    echo '<td><span class="' . $prioriteClass . '">' . $Priorite . '</span></td>';
                    echo '<td>' . $Budget_heure . '</td>';
                    ...
                    echo '<option value="Livré"' . ($Statut === 'Livré' ? ' selected' : '') . '>Livré</option>';
                    ...
                    switch ($Type_dossier) {
                      ...
                      case 'Normal':
                        ...
                        break;
                      case 'Faible':
                        $typeClass = 'badge bg-light';
                        break;
                      case 'Autre':
                        $typeClass = 'badge bg-light';
                        break;
                      default:
                        $typeClass = ''; // No special class for other cases
                    }
                    echo '<td><span class="' . $typeClass . '">' . $Type_dossier . '</span></td>';
                    echo '</tr>';
                    }
                    echo '</tbody>';
                    echo '</table>';
